...
export default function PDFMerger() {
  ...
  async function downloadPdf() {
    ...
    const fileDataArray: { file: File; buffer: ArrayBuffer | string }[] = await readFile();
    const pdfPagesArray: Array<PDFPage | PDFPage[] | undefined> = await addPages(fileDataArray);
  ...
  function addPages(fileDataArray: { file: File; buffer: ArrayBuffer | string }[]): Promise<Array<PDFPage | undefined | PDFPage[]>> {
    ...
    <>
      <div className="flex items-center justify-center p-12">
        <div className="mx-auto w-full max-w-[850px] bg-white py-6 px-9 dark:bg-[#27292C] rounded-md shadow-md dark:shadow-none duration-200">
              ...
              <div className="flex items-center justify-center">
                {fileList.length > 0 && (
                  <div id="text no-file" className="text-center">
                    ...
                    <DraggableList type="image" fileList={fileList} setFileList={setFileList} />
                  </div>
                )}
                {fileList.length === 0 && (
                  <div id="text no-file" className="text-center">
          ...
          <div>
            <button onClick={downloadPdf} className="hover:shadow-form w-full rounded-md bg-[#774FE9] py-3 px-8 text-center text-base font-semibold text-white outline-none">
              Convert File
            </button>
          </div>
        </div>
